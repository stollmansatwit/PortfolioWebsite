...
(function () {
  ...
  function changeBackgroundWithBands() {
    ...
    // Step 1: Check band positions to determine if we need to animate up or down
    if (currentIndex === 5) {
      // Step 2: Animate all bands up (using scroll.js function) if not already
      setTimeout(() => {
        window.animateAllBands('up');
      }, 500);
    }
    if (currentIndex === 0) {
      // Step 3: Wait for up animation to complete, then change background (Also change the button message to indicate loading state)
      ...
      // Step 4: Animate all bands back down
      setTimeout(() => {
        window.allBandsDown();
      }, 2000);
      // Step 5: Re-enable scroll input after down animation completes
    ...
    function createButton() {
      ...
      <span class="tooltip" style="font-family: 'Wix Madefor Display', sans-serif !important;">change background</span>
    `;
